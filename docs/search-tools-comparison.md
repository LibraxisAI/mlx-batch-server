...
|---------|-------------|-------|----------|
| `loct find <pattern>` | Symbol + semantic search | ✅ | Find functions, classes by name or similarity |
...
1. **Monkey-patching patterns**: compat.py exports symbols injected into `sys.modules` - loct sees them as dead
